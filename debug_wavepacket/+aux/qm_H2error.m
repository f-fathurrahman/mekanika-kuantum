...
prt.disp (['Value of A-shift : ' num2str(reduce.H2error.A_shift)])

if ~isfield(reduce.H2error,'BN_scale')
    reduce.H2error.BN_scale = 1;
end
prt.disp(['Scaling B-vector, N-matrix: ' num2str(reduce.H2error.BN_scale)])

if ~isfield (reduce.H2error,'method')
    reduce.H2error.method='iter';
end
prt.disp(['Method for GLYAP solving: ' reduce.H2error.method])
...
if ~isempty (truncate)
    ...
    for k=1:length(truncate)
        ...
        prt.disp('   ')
        prt.disp('--------------------------------')
        prt.disp('Calculate error norm (BT method)')
        prt.disp('--------------------------------')
        prt.disp('   ')
...
if ~isempty (singular)
    ...
    for k=1:length(singular)
        ...
        prt.disp('   ')
        prt.disp('--------------------------------')
        prt.disp('Calculate error norm (SP method)')
        prt.disp('--------------------------------')
        prt.disp('   ')
...
if ~isempty (H2reduce)
    ...
    for k=1:length(H2reduce)
        ...
        prt.disp('   ')
        prt.disp('--------------------------------')
        prt.disp('Calculate error norm (H2 method)')
        prt.disp('--------------------------------')
        prt.disp('   ')
...
switch reduce.H2error.method
    ...
    otherwise
        prt.error (['Wrong choice of GLYAP solver method : ' reduce.H2error.method])
end
...
H2err = sqrt(H2err);
prt.disp(['H2 error norm = ', num2str(real(H2err))])
prt.disp('   ')
