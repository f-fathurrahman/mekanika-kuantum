...
function pot_eig_N ( efield, save_trafo )
global hamilt space
...
for gr=1:space.n_tot
    ...
    for m=1:hamilt.coupling.n_eqs
            ...
            for p = 1:length(hamilt.dip)
                if abs(efield(p))>0
                    if ~isempty (hamilt.dip{p})
                        if ~hamilt.dip{p}{m,m}.empty
                            pot_mat(m,m) = pot_mat(m,m) - efield(p) * hamilt.dip{p}{m,m}.dvr(gr);
                        end
                    end
                end
            end
                    ...
                    for q = p:size(hamilt.pol,2)
                        if abs(efield(q))>0
                            if ~isempty (hamilt.pol{p,q})
                                if ~hamilt.pol{p,q}{m,m}.empty
                                    pot_mat(m,m) = pot_mat(m,m) - efield(p)*efield(q)/2 * hamilt.pol{p,q}{m,m}.dvr(gr);
                                end
                            end
                        end
                    end
        ...
        for n=m+1:hamilt.coupling.n_eqs
                    ...
                    if abs(efield(p))>0
                        if ~isempty (hamilt.dip{p})
                            if  ~hamilt.dip{p}{m,n}.empty
                                pot_mat(m,n) = pot_mat(m,n) - efield(p) * hamilt.dip{p}{m,n}.dvr(gr);
                            end
                        end
                    end
                end
                            ...
                            if abs(efield(q))>0
                                if ~isempty (hamilt.pol{p,q})
                                    if  ~hamilt.pol{p,q}{m,n}.empty
                                        pot_mat(m,n) = pot_mat(m,n) - efield(p)*efield(q)/2 * hamilt.pol{p,q}{m,n}.dvr(gr);
                                    end
                                end
                            end
                        end
            ...
            % Make potential matrix symmetric|Hermitian
