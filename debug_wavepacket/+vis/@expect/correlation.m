...
if time.steps.s_number * time.steps.m_number<=1000
    ...
    % Too many time steps: plot 3 curves showing abs(C), Re(C), Im(C) instead
else
    ...
    h = plot ( time.steps.s_grid, real(time.steps.acf) );
    set(h, ...
        'LineStyle', obj.patterns{2}, ...
        'Color',     'black', ...
        'LineWidth', obj.l_thick)
    
    h = plot ( time.steps.s_grid, imag(time.steps.acf) );
    set(h, ...
...
% Axes and labels
axis ( [ 0 time.steps.t_total -1.1 1.1 ] )
set(gca, ...
    'LineWidth',  obj.l_thick, ...
    'FontName',   obj.f_name, ...
