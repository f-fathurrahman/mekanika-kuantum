...
function stop = monitor_function(x, optimValues, state)
    ...
    if termination_conditions.loop_count <= N_eval_counter;
      termination_reason = OC.const.termination_reason.loop_count;
      stop = true;
    end
    
    if termination_conditions.wall_time_to_stop <= (now()-wall0)*(24*60*60)
      termination_reason = OC.const.termination_reason.wall_time;
      stop = true;
    end
    
    if termination_conditions.cputime_to_stop <= (cputime()-cpu0)
      termination_reason = OC.const.termination_reason.cpu_time;
      stop = true;
    end
    
    if termination_conditions.gradient_norm_min >= last_grad_norm
      termination_reason = OC.const.termination_reason.gradient_norm;
      stop = true;
    end
    
    if termination_conditions.goal <= get_current_value();
...
% If BFGS cannot find the goal, this is because the gradients are pointing nowhere useful
termination_reason = OC.const.termination_reason.gradient_norm;
