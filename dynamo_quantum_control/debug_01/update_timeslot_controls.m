...
% update_timeslot_controls will set the new control values
% (the values in new_control_values for which subspace_mask is true)
%
global OC;

disp('<div> ENTER update_timeslot_controls')
...
if nargin == 1
  ...
  disp('size(subspace_mask) = '); disp(size(subspace_mask));
  disp('size(ncv) = '); disp(size(ncv));
  disp('size(new_control_values) = '); disp(size(new_control_values))
...
disp('</div> EXIT update_timeslot_controls')
