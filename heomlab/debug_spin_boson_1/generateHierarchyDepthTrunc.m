function [ado_indices,ado_gammas,lower_indices,upper_indices, ...
...
fprintf('\n<div> ENTER generateHierarchyDepthTrunc\n');
...
%fprintf(file_id, 'n_modes = '); fprintf(file_id, n_modes);
  ...
  for k = 1:n_modes
    for r = 1:n_lower
      ...
      % if it isn't new, just store the coupling information
        ...
        %disp('Pass her 62, just store coupling info'); % many
        lower_indices = [lower_indices; J_r];
        K = index + n_below;
        upper_indices = [upper_indices; K];
        coupled_mode_indices = [coupled_mode_indices; k];
...
fprintf('\n</div> EXIT generateHierarchyDepthTrunc\n');
