...
n=size(A,1);
prt.disp ('Solving generalized Lyapunov equation by biconjugate gradients')
...
prt.disp(['GLYAP2 error norm = ' num2str(norm(Deviate))]);
prt.disp ('   ')
end
...
if strcmp(transp_flag,'transp')
    if info.control_system
        Y = lyap(A',X); % from Matlab's control system toolbox
    else
        Y = math.lyap2solve(A',X); % from MPI in Magdeburg
    end
