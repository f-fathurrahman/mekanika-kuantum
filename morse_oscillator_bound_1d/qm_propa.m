...
function qm_propa (string1, param1, param2, param3)

%% Choice of propagators
global expect info state time
...
if isa(state,'wave')                     % quantum wavefunctions, represented on grids
    ...
    switch lower(string1)
        ...
        case 'diff_4'                    % Fourth order differencing
            time.propa = tmp.wave.diff_4;
        ...
        case 'cheby_imag'                % Chebychev polynomials in imaginary time
            time.propa = tmp.wave.cheby_imag (param1, param2, param3);
        otherwise
...
for step = 1 : time.steps.m_number
    ...
    if strcmpi(info.program,'qm_propa') && isa (time.propa,'tmp.wave.cheby_imag')
        if step>1 && abs(expect.total(step)-expect.total(step-1)) < time.propa.threshold
            prt.disp('***************************************************************')
            prt.disp('Convergence reached before expiration of time loop')
            prt.disp('***************************************************************')
            prt.disp(' ')
            break
        end
