...
if s(1)~=t(2)
    prt.error ('Wrong dimensions s1,t2 in testing pseudo-inverse')
end
if s(2)~=t(1)
    prt.error ('Wrong dimensions s2,t1 in testing pseudo-inverse')
end
...
STS = S*T*S - S;
TST = T*S*T - T;
prt.disp ('Transformation matrices S and T (pseudo-inverse)')
prt.disp (['Dimension of S : ' int2str(s(1)) ' x ' int2str(s(2))])
prt.disp (['Dimension of T : ' int2str(t(1)) ' x ' int2str(t(2))])
prt.disp (['||( ST-I)|| / ||I|| = ' num2str(norm(STI,'fro')/norm(I1,'fro'))])
prt.disp (['||( TS-I)|| / ||I|| = ' num2str(norm(TSI,'fro')/norm(I2,'fro'))])
prt.disp (['||(STS-S)|| / ||S|| = ' num2str(norm(STS,'fro')/norm(S, 'fro'))])
prt.disp (['||(TST-T)|| / ||T|| = ' num2str(norm(TST,'fro')/norm(T, 'fro'))])
prt.disp ('   ')
