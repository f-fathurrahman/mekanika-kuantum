...
function FluctuatorCallback(F::MyOpenQuantumBase.FluctuatorLiouvillian, initialize, save_positions)
    ...
    affect! = function (integrator)
        F.n = sum(F.b0, dims=1)[:]
        MyOpenQuantumBase.next_state!(F)
        u_modified!(integrator, false)
    end

    initialize_fluc = function (cb, u, t, integrator)
        MyOpenQuantumBase.reset!(F, initialize)
        u_modified!(integrator, false)
    end
...
function LindbladJumpCallback(save_positions)
    ...
    affect! = function (integrator)
        A = MyOpenQuantumBase.lindblad_jump(
            integrator.p.L,
            integrator.u,
            integrator.p,
            integrator.t,
