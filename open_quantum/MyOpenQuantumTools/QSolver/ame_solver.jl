...
function solve_ame(
    ...
    if one_sided == false
        L = MyOpenQuantumBase.davies_from_interactions(A.interactions, ω_hint, lambshift, lambshift_kwargs)
    else
        L = MyOpenQuantumBase.onesided_ame_from_interactions(A.interactions, ω_hint, lambshift, lambshift_kwargs)
    end
    if vectorize
...
function build_ensemble_ame(
    ...
    flist = MyOpenQuantumBase.fluctuator_from_interactions(A.interactions)
    dlist = MyOpenQuantumBase.davies_from_interactions(A.interactions, ω_hint, lambshift, lambshift_kwargs)
...
function solve_ame(
    ...
    # build gaps for AME
    gap_idx = MyOpenQuantumBase.build_gap_indices(2*π*w, digits, sigdigits, cutoff, lvl)
    # prepare for the initial state
    u0 = build_u0(A.u0, :m, vectorize=vectorize)
    u0 = v' * u0 * v

    if one_sided == false
        L = MyOpenQuantumBase.davies_from_interactions(gap_idx, inters, ω_hint, lambshift, lambshift_kwargs)
    else
    ...
    f = MyOpenQuantumBase.build_diffeq_liouvillian(H, [], L, lvl, digits=digits, sigdigits=sigdigits)
    p = ODEParams(f, float(tf), A.annealing_parameter)
    prob = ODEProblem(f, u0, tspan, p)
    solve(prob; alg_hints=[:nonstiff], kwargs...)
end
...
function build_ensemble_ame(
    ...
    flist = MyOpenQuantumBase.fluctuator_from_interactions(A.interactions)
    dlist = MyOpenQuantumBase.davies_from_interactions(gap_idx, inters, ω_hint, lambshift, lambshift_kwargs)
    L = MyOpenQuantumBase.build_diffeq_liouvillian(H, [], [dlist; flist], lvl, digits=digits, sigdigits=sigdigits)
    cb = build_jump_callback(dlist, flist, initializer, save_positions)
    p = ODEParams(L, tf, A.annealing_parameter)
    update_func = function (cache, u, p, t)
        update_cache!(cache, p.L, p, t)
