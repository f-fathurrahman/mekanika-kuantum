...
function solve_lindblad(
    ...
    L = MyOpenQuantumBase.lindblad_from_interactions(A.interactions)
    if vectorize
...
function build_ensemble_lindblad(
    ...
    u0 = build_u0(A.u0, :v)
    L = DiffEqLiouvillian(A.H, [], MyOpenQuantumBase.lindblad_from_interactions(A.interactions), size(A.H, 1))
