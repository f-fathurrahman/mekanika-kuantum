...
function solve_redfield(
    ...
    L = MyOpenQuantumBase.redfield_from_interactions(A.interactions, unitary, Ta, int_atol, int_rtol)
...
function solve_cgme(
    ...
    u0 = build_u0(A.u0, :m, vectorize=vectorize)
    L = MyOpenQuantumBase.cg_from_interactions(A.interactions, unitary, tf, Ta,
            int_atol, int_rtol)
    R = DiffEqLiouvillian(A.H, [], L, size(A.H, 1))
    update_func = function (A, u, p, t)
        update_vectorized_cache!(A, p.L, p, t)
...
function solve_ule(
    ...
    u0 = build_u0(A.u0, :m, vectorize=vectorize)
    L = MyOpenQuantumBase.ule_from_interactions(A.interactions, unitary, Ta, int_atol, int_rtol)
    R = DiffEqLiouvillian(A.H, [], L, size(A.H, 1))
...
function build_ensemble_redfield(
    ...
    u0 = build_u0(A.u0, :m, vectorize=vectorize)
    stocs = MyOpenQuantumBase.fluctuator_from_interactions(A.interactions)
    reds = MyOpenQuantumBase.redfield_from_interactions(A.interactions, unitary, Ta, int_atol, int_rtol)
    if isempty(stocs)
