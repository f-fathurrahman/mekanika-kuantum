using MyOpenQuantumBase, Test
# test suite for Ohmic bath
η = 1e-4
ωc = 8 * pi
β = 1 / 2.23
bath = MyOpenQuantumBase.OhmicBath(η, ωc, β)
cfun_test = MyOpenQuantumBase.build_correlation(bath)
γfun_test = MyOpenQuantumBase.build_spectrum(bath)
...
@test S(0.0, bath) ≈ MyOpenQuantumBase.lambshift_cpvagk(0.0, (x)->γ(x, bath)) atol = 1e-4 rtol = 1e-4
# the following test is kept as a consistency check
@test MyOpenQuantumBase.lambshift_cpvagk(0.0, (x)->γ(x, bath)) ≈ -0.0025132734115775254 rtol = 1e-4
...
rtn = MyOpenQuantumBase.SymetricRTN(2.0, 2.0)
...
@test S(0.0, bath) ≈ MyOpenQuantumBase.lambshift_cpvagk(0.0, (x)->γ(x, bath)) atol = 1e-4 rtol = 1e-4
# the following test is kept as a consistency check
@test_broken MyOpenQuantumBase.lambshift_cpvagk(0.0, (x)->γ(x, bath)) ≈ -0.2872777516270734
...
γm = MyOpenQuantumBase.build_spectrum(cbath)
@test γm[1, 1](0.0) == 0
@test γm[2, 2](0.0) == 0
@test γm[1, 2](0.5) == exp(-0.5)
@test γm[2, 1](-0.5) == exp(-0.5*0.8)

@test_throws ArgumentError MyOpenQuantumBase.build_correlation(cbath)
lambfun_1 = MyOpenQuantumBase.build_lambshift([0.0], false, cbath, Dict())
@test lambfun_1[1,1](0.0) == 0
@test lambfun_1[2,2](0.1) == 0
@test lambfun_1[1,2](0.5) == 0
@test lambfun_1[2,2](1.0) == 0

lambfun_2 = MyOpenQuantumBase.build_lambshift([], true, cbath, Dict())
lambfun_3 = MyOpenQuantumBase.build_lambshift(range(-5,5,length=1000), true, cbath, Dict())
lambfun_4 = MyOpenQuantumBase.build_lambshift([], true, cbath, Dict(:order=>1))
lambfun_5 = MyOpenQuantumBase.build_lambshift([0.0, 0.01], true, cbath, Dict(:order=>1))
