...
funcs = [(x) -> x, (x) -> 1 - x]
test_diag_operator = MyOpenQuantumBase.DiagonalOperator(funcs)
@test test_diag_operator(0.5) == MyOpenQuantumBase.Diagonal([0.5, 0.5])

test_geometric_operator = MyOpenQuantumBase.GeometricOperator(((x) -> -1.0im * x))
@test test_geometric_operator(0.5) == [0 0.5im; -0.5im 0]
@test_throws ArgumentError MyOpenQuantumBase.GeometricOperator((x) -> x, (x) -> 1 - x)
