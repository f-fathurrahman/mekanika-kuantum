using MyOpenQuantumBase, Test, Random
...
davies = MyOpenQuantumBase.DaviesGenerator(coupling, gamma, lamb)
onesided = MyOpenQuantumBase.OneSidedAMELiouvillian(coupling, MyOpenQuantumBase.SingleFunctionMatrix(gamma), MyOpenQuantumBase.SingleFunctionMatrix(lamb), [(1, 1)])
ops = [2π * (σz ⊗ σi + σi ⊗ σz)]
...
ρ = ψ * ψ'
u = v' * ρ * v
g_idx = MyOpenQuantumBase.GapIndices(w, 8, 8)

# ## Tests
# Test density matrix update function for `DaviesGenerator` type
dρ = MyOpenQuantumBase.ame_update_test(ops, ρ, w, v, gamma, lamb)
du = zeros(ComplexF64, (4, 4))
davies(du, u, g_idx, v, 0.5)
@test v * du * v' ≈ dρ atol = 1e-6 rtol = 1e-6

onesided_dρ = MyOpenQuantumBase.onesided_ame_update_test(ops, ρ, w, v, gamma, lamb)
du = zeros(ComplexF64, (4, 4))
onesided(du, u, g_idx, v, 0.5)
@test v * du * v' ≈ onesided_dρ atol = 1e-6 rtol = 1e-6

onesided_dρ = MyOpenQuantumBase.onesided_ame_update_test([v * op * v' for op in ops], ρ, w, v, gamma, lamb)
du = zeros(ComplexF64, (4, 4))
onesided(du, u, g_idx, 0.5)
@test du ≈ v' * onesided_dρ * v atol = 1e-6 rtol = 1e-6

cache = zeros(ComplexF64, (4, 4))
exp_effective_H = MyOpenQuantumBase.ame_trajectory_Heff_test(ops, w, v, gamma, lamb)
...
exp_effective_H = MyOpenQuantumBase.ame_trajectory_Heff_test(ops, w, v, gamma, lamb) + H(0.5)
cache = zeros(ComplexF64, 4, 4)
...
jump_op = MyOpenQuantumBase.lindblad_jump(ame_op, ψ, p, 1)
...
gapidx_const = MyOpenQuantumBase.build_gap_indices(wc, 8, 8, Inf, 4)
davies_const = MyOpenQuantumBase.build_const_davies(rotate(couplings_const, vc), gapidx_const, gamma, lamb)

# Test density matrix update function for `ConstDaviesGenerator` type
dρ = MyOpenQuantumBase.ame_update_test(couplings_const(0), ρc, wc, vc, gamma, lamb)
du = zeros(ComplexF64, (4, 4))
davies_const(du, uc, nothing, 0)
@test dρ ≈ vc * du * vc'

effective_H = MyOpenQuantumBase.ame_trajectory_Heff_test(couplings_const(0), wc, vc, gamma, lamb)
cache = zeros(ComplexF64, (4, 4))
update_cache!(cache, davies_const, nothing, 0)
@test -1.0im * effective_H ≈	vc * cache * vc'

# Test for lindblad_jump
Lc = MyOpenQuantumBase.build_diffeq_liouvillian(Hamiltonian(MyOpenQuantumBase.sparse(MyOpenQuantumBase.Diagonal(wc)), unit=:ħ), [], [davies_const] , 4)

jump_op = MyOpenQuantumBase.lindblad_jump(Lc, vc'*ψc, p, 1)
@test size(jump_op) == (4, 4)
...
dρ = MyOpenQuantumBase.ame_update_test(ops, ρ, w, v, gamma, lamb)
exp_effective_H =
    MyOpenQuantumBase.ame_trajectory_Heff_test(ops, w, v, gamma, lamb) + v * Diagonal(w) * v'
...
#= ============ Tests ============ =#
dρ = MyOpenQuantumBase.ame_update_test(coupling(0.4), ρ, w, v, gamma, lamb)
p = ODEParams(H, 2.0, (tf, t) -> t / tf)
ame_op = DiffEqLiouvillian(H, [davies], [], 4)
du = zeros(ComplexF64, (4, 4))
ame_op(du, ρ, p, 0.4 * 2)
...
D = MyOpenQuantumBase.davies_from_interactions(InteractionSet(Interaction(coupling, cbath)), 1:10, false, Dict())[1]
@test typeof(D) <: MyOpenQuantumBase.CorrelatedDaviesGenerator
du = zeros(ComplexF64, 2, 2)
ρ = [0.5 0; 0 0.5]
ω = [1, 2]
g_idx = MyOpenQuantumBase.GapIndices(ω, 8, 8)
D(du, ρ, g_idx, 0.5)
@test du ≈ zeros(2, 2)
