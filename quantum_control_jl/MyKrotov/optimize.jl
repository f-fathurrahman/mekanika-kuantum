using MyQuantumControl.MyQuantumPropagators.Generators: Operator
using MyQuantumControl.MyQuantumPropagators.Controls: discretize, evaluate
using MyQuantumControl.MyQuantumPropagators.Interfaces: supports_inplace
using MyQuantumControl.MyQuantumPropagators: prop_step!, reinit_prop!, propagate

using MyQuantumControl.MyQuantumPropagators.Storage:
    write_to_storage!, get_from_storage!, get_from_storage

using MyQuantumControl.Functionals: make_chi, taus!
using MyQuantumControl: set_atexit_save_optimization
using MyQuantumControl: @threadsif, Trajectory
using MyQuantumControl.MyQuantumPropagators: _StoreState

using LinearAlgebra
using Printf

import MyQuantumControl: optimize, make_print_iters
...
"""
optimize(problem, method::Val{:MyKrotov}) = optimize_krotov(problem)
optimize(problem, method::Val{:Mykrotov}) = optimize_krotov(problem)
...
make_print_iters(::Val{:MyKrotov}; kwargs...) = make_krotov_print_iters(; kwargs...)
make_print_iters(::Val{:Mykrotov}; kwargs...) = make_krotov_print_iters(; kwargs...)
