...
using MyQuantumPropagators: hamiltonian, propagate
using MyQuantumPropagators.Controls: discretize
...
function debug_main()
    ...
    ϵ(t) = blackman_pulse(t; ω, E₀, ti, tf)
    H, ket0, ket1 = ham_and_states(ω, ϵ);
    states = propagate(ket0, H, tlist; method=:expprop, storage=true)

    # H is a Generator
    # evaluate Generator at a time, we obtain and `Operator`
    # Convert `Operator` to Array, we get a usual Matrix
